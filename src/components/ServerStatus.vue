		...
		async updateStatus() {
			try {
				const response = await fetch(
					`https://api.mcstatus.io/v2/status/java/${this.host}:${this.port}`
				);
				const data = await response.json();
				this.serverStatus = data.online
					? '<p class="part_STATUS_online">Online</p>'
					: '<p class="part_STATUS_offline">Offline</p>';
			} catch (error) {
				console.error("Error fetching server status:", error);
				this.serverStatus =
					"Error fetching server status: " + error.message;
			}
		},
	},
};
